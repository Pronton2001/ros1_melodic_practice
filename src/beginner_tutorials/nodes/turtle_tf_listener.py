...
if __name__ == '__main__':
    ...
    # The first waitForTransform() will wait until the /turtle2 frame is broadcast on 
    # tf before trying to waitForTransform() at time now.
    listener.waitForTransform("/turtle2", "/carrot1", rospy.Time(), rospy.Duration(1.0))
    while not rospy.is_shutdown():
        # try:
        #     now = rospy.Time.now()  - rospy.Duration(5.0)# same as rospy.Time(0)
